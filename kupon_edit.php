...
if (!isset($_GET['id'])) {
    header('Location: kupon-proses.php');
    exit;
}

$id = (int) $_GET['id'];

// Ambil data bayar_kupon beserta nama dari tabel kupon
$stmt = $pdo->prepare("
    SELECT bk.*, k.nama, k.nomor_kupon
    FROM bayar_kupon bk
    JOIN kupon k ON bk.id_kupon = k.id
    WHERE bk.id = ?
    LIMIT 1
");
$stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$tanggal) {
    ...
    } elseif (!is_numeric($bayar_jumlah) || $bayar_jumlah <= 0) {
        $error = "Jumlah bayar harus berupa angka lebih dari 0.";
    } else {
        ...
        if (!empty($_FILES['bukti']['name'])) {
            ...
            $namaKuponSlug = slugify($bayar['nama']);
            $tanggalFile = date('Ymd', strtotime($tanggal));
            $filename = "bayar-kupon-{$namaKuponSlug}-{$tanggalFile}." . $ext;
        ...
        if (empty($error)) {
            try {
                $stmtUpd = $pdo->prepare("
                    UPDATE bayar_kupon
                    SET bayar = ?, tanggal = ?, bukti = ?
                    WHERE id = ?
                ");
                $stmtUpd->execute([$bayar_jumlah, $tanggal, $bukti, $id]);
                header('Location: kupon-proses.php?updated=1');
                exit;
        ...
        <main class="app-main">
            ...
            <div class="app-content-header">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <h3 class="mb-0">Edit Pembayaran Kupon</h3>
                        </div>
                        <div class="col-sm-6">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="card card-primary card-outline mb-4">
                                ...
                                <div class="card-header">
                                    <div class="card-title">Pembayaran Kupon Information</div>
                                </div>
                                <!--end::Header-->
                                <?php if ($error): ?>
                                ...
                                <form method="POST" enctype="multipart/form-data">
                                    <div class="card-body">
                                        ...
                                        <div class="mb-3">
                                            <label class="form-label">Nama Pemilik Kupon</label>
                                            <input type="text" class="form-control" value="<?= htmlspecialchars($bayar['nama']) ?>" disabled>
                                            <small class="text-muted">No. Kupon: <?= htmlspecialchars($bayar['nomor_kupon']) ?></small>
                                        </div>

                                        <div class="mb-3">
                                            <label class="form-label">Jumlah Bayar (Rp)</label>
                                            <input type="number" name="bayar" class="form-control" required min="1"
                                                value="<?= htmlspecialchars($bayar['bayar']) ?>">
                                        </div>

                                        <div class="mb-3">
                                            <label class="form-label">Tanggal Pembayaran</label>
                                            <input type="date" name="tanggal" class="form-control" required
                                                value="<?= htmlspecialchars(date('Y-m-d', strtotime($bayar['tanggal']))) ?>">
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <?php if (!empty($bayar['bukti'])): ?>
                                                <p class="mt-2">File saat ini:
                                                    <a href="uploads/<?= htmlspecialchars($bayar['bukti']) ?>" target="_blank">
                                                        <?= htmlspecialchars($bayar['bukti']) ?>
                                                    </a>
                                                </p>
                                            <?php endif; ?>
                                        </div>
                                    ...
                                    <div class="card-footer">
                                        ...
                                        <a href="kupon-proses.php" class="btn btn-secondary float-end">Kembali</a>
                                    </div>
                                </form>
                                <!--end::Form-->
                            </div>
