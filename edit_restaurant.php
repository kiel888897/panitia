...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        $target_directory = 'uploads/restaurants/'; // Folder simpan foto

        if (!is_dir($target_directory)) {
            mkdir($target_directory, 0755, true); // Buat folder jika belum ada
        }
        ...
        if (move_uploaded_file($photo['tmp_name'], $target_path)) {
            ...
            if (!empty($restaurant['photo'])) {
                $old_photo_path = $target_directory . $restaurant['photo'];
                if (file_exists($old_photo_path)) {
                    unlink($old_photo_path); // Hapus file lama
                }
            }
            // photo_filename sudah berisi nama file baru
        } else {
            $error = 'Failed to upload photo.';
            $photo_filename = $restaurant['photo']; // fallback ke file lama jika gagal upload
        }
    } else {
        $photo_filename = $restaurant['photo']; // Tidak upload baru, tetap pakai foto lama
    }
    ...
    if (!empty($_FILES['menu_file']['name'])) {
        ...
        if (move_uploaded_file($menu_file['tmp_name'], $menu_target_path)) {
            ...
            if (!empty($restaurant['menu'])) {
                $old_menu_path = $menu_target_directory . $restaurant['menu'];
                if (file_exists($old_menu_path)) {
                    unlink($old_menu_path); // Hapus file lama
                }
            }
            // Gunakan nama file baru
            // $menu_file_filename sudah berisi nama file baru
        } else {
            $error = 'Failed to upload menu file.';
            $menu_file_filename = $restaurant['menu']; // fallback ke file lama jika gagal upload
        }
    } else {
        $menu_file_filename = $restaurant['menu']; // Tidak ada upload baru, pakai file lama
    }

    if ($name && $latitude) {
        $stmt = $pdo->prepare("UPDATE restaurants SET name = ?, location = ?, description = ?, address = ?, phone_resto = ?, phone_manager = ?, phone_owner = ?, bank_account = ?, number_bank = ?, food_type = ?, cost_delivery = ?, latitude = ?, longitude = ?, photo = ?, link_menu = ?, menu = ?, status = ?, slug = ? WHERE id = ?");
        $stmt->execute([
            $name,
            $location,
            $description,
        ...
        $error = 'Name, Latitude fields are required.';
    }
}
?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-end">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Edit Restaurants</li>
                            </ol>
                        </div>
                    </div>
                    <!--end::Row-->
                                ...
                                <form method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="card-body">
                                        <div class="mb-3">
                                            ...
                                            <input
                                                type="text" class="form-control" name="name" id="name" aria-describedby="name" value="<?= htmlspecialchars($restaurant['name']) ?>" required />
                                        </div>
                                        <div class="mb-3">
                                            <label for="location" class="form-label">Location</label>
                                            <select class="form-select" name="location" id="location">
                                                <option disabled value="">Choose ...</option>
                                                <option value="nembrala" <?= ($restaurant['location'] === 'nembrala') ? 'selected' : '' ?>>Nembrala</option>
                                                <option value="boa" <?= ($restaurant['location'] === 'boa') ? 'selected' : '' ?>>Boa</option>
                                                <option value="sedoin" <?= ($restaurant['location'] === 'sedoin') ? 'selected' : '' ?>>Sedoin</option>
                                                <option value="tenggoin" <?= ($restaurant['location'] === 'tenggoin') ? 'selected' : '' ?>>tenggoin</option>
                                            </select>
                                            <div class="invalid-feedback">Please select a valid Food Type.</div>
                                        </div>

                                        <div class=" mb-3">
                                            <label for="address" class="form-label">Address</label>
                                            <textarea class="form-control" name="address" id="address" aria-label="address"><?= htmlspecialchars($restaurant['address']) ?></textarea>
                                        </div><?php
                                                $selected_food_types = explode(',', $restaurant['food_type'] ?? '');
                                                ?>
                                                ...
                                                <div class="col-6 col-md-4">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="food_type[]" value="general" id="foodGeneral"
                                                            <?= in_array('general', $selected_food_types) ? 'checked' : '' ?>>
                                                ...
                                                <div class="col-6 col-md-4">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="food_type[]" value="vegetarian" id="foodVegetarian"
                                                            <?= in_array('vegetarian', $selected_food_types) ? 'checked' : '' ?>>
                                                ...
                                                <div class="col-6 col-md-4">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="food_type[]" value="seafood" id="foodSeafood"
                                                            <?= in_array('seafood', $selected_food_types) ? 'checked' : '' ?>>
                                                ...
                                                <div class="col-6 col-md-4">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="food_type[]" value="indonesian" id="foodIndonesian"
                                                            <?= in_array('indonesian', $selected_food_types) ? 'checked' : '' ?>>
                                                ...
                                                <div class="col-6 col-md-4">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="food_type[]" value="italian" id="foodItalian"
                                                            <?= in_array('italian', $selected_food_types) ? 'checked' : '' ?>>
                                                ...
                                                <div class="col-6 col-md-4">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="food_type[]" value="western" id="foodWestern"
                                                            <?= in_array('western', $selected_food_types) ? 'checked' : '' ?>>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="link_menu" id="link_menu" class="form-control" aria-describedby="link_menu" value="<?= htmlspecialchars($restaurant['link_menu']) ?>">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <?php if ($restaurant['menu']) : ?>
                                                <a href="uploads/menus/<?= htmlspecialchars($restaurant['menu']) ?>" target="_blank">Download Menu</a>

                                            <?php else : ?>
                                                <p class="text-muted fst-italic">
                                                    <i class="bi bi-exclamation-circle me-1"></i> Menu is not available at the moment.
                                                </p>
                                            <?php endif; ?>
                                        </div>
                                        <div class="mb-3">
                                            <label for="description" class="form-label">Description</label>
                                            <textarea class="form-control" name="description" id="description" aria-label="description"><?= htmlspecialchars($restaurant['description']) ?></textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label for="phone_resto" class="form-label">Phone Restaurant</label>
                                            <input type="text" name="phone_resto" id="phone_resto" class="form-control" placeholder="e.g. 6281234567890" aria-describedby="phone_resto" value="<?= htmlspecialchars($restaurant['phone_resto']) ?>" />
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="phone_manager" id="phone_manager" class="form-control" placeholder="e.g. 6281234567890" aria-describedby="phone_manager" value="<?= htmlspecialchars($restaurant['phone_manager']) ?>">
                                        </div>
                                        <div class="mb-3">
                                            <label for="phone_owner" class="form-label">Phone Owner</label>
                                            <input type="text" name="phone_owner" id="phone_owner" class="form-control" placeholder="e.g. 6281234567890" aria-describedby="phone_owner" value="<?= htmlspecialchars($restaurant['phone_owner']) ?>">
                                        </div>
                                        <div class="mb-3">
                                            <label for="bank_account" class="form-label">Bank</label>
                                            <input type="text" name="bank_account" id="bank_account" class="form-control" aria-describedby="bank_account" value="<?= htmlspecialchars($restaurant['bank_account']) ?>">
                                        </div>
                                        <div class="mb-3">
                                            <label for="number_bank" class="form-label">Number Bank Account</label>
                                            <input type="number" name="number_bank" id="number_bank" class="form-control" aria-describedby="number_bank" value="<?= htmlspecialchars($restaurant['number_bank']) ?>">
                                        </div>
                                        <div class="mb-3">
                                            <label for="cost_delivery" class="form-label">Cost Delivery</label>
                                            <input type="number" name="cost_delivery" id="cost_delivery" class="form-control" aria-describedby="cost_delivery" value="<?= htmlspecialchars($restaurant['cost_delivery']) ?>">
                                        </div>
                                        <div class="row g-3 mb-3">
                                            <div class="col-md-6">
                                                <label for="latitude" class="form-label">Latitude</label>
                                                <input type="text" name="latitude" id="latitude" class="form-control" aria-describedby="latitude" value="<?= htmlspecialchars($restaurant['latitude']) ?>" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label for="longitude" class="form-label">Longitude</label>
                                                <input type="text" name="longitude" id="longitude" class="form-control" aria-describedby="longitude" value="<?= htmlspecialchars($restaurant['longitude']) ?>" required>
                                            </div>
                                        </div>
                                        ...
                                        <?php if (!empty($restaurant['photo'])): ?>
                                            <div class="mb-3">
                                                <img src="uploads/restaurants/<?= htmlspecialchars($restaurant['photo']) ?>" alt="<?= htmlspecialchars($restaurant['name']) ?>" style="max-width: 200px; height: auto;">
                                            </div>
                                        <?php endif; ?>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" name="status" id="status">
                                                <option disabled value="">Choose ...</option>
                                                <option value="activ" <?= ($restaurant['status'] === 'activ') ? 'selected' : '' ?>>Activ</option>
                                                <option value="draft" <?= ($restaurant['status'] === 'draft') ? 'selected' : '' ?>>Draft</option>
                                            </select>
                                            <div class="invalid-feedback">Please select a valid Food Type.</div>
                                        </div>
